...
public class Main {
    ...
    public static void main(String[] args) {
        String harryPotter = "Mr. and Mrs. Dursley, of number four, Privet Drive, were proud to say that they were " +
                "perfectly normal, thank you very much. They were the last people you'd expect to be involved in " +
           ...
           String buffer = harryPotter.replaceAll("[^A-Z,a-z ]", "");
        System.out.println(buffer);
        String[] array = buffer.split(" ");
        System.out.println(array[5]);
        System.out.println(array.length);
        for (int i = 0; i <array.length ; i++) {
            if (array[i].length() == 4 && array[i].charAt(0) == 'a' || (array[i].length() == 7 && array[i].charAt(0) == 'a') ){
